    ...
    <div style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', lineHeight: 1.8, color: 'var(--color-text)' }}>
      ...
      <section id="practices" style={{ padding: 'clamp(4rem, 7vw, 6.5rem) max(1.5rem, 8vw)', background: 'var(--color-cream)' }}>
        <div style={{ maxWidth: '1100px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
            {[
              { name: 'TRE Wall Sit Protocol', time: '20\u201330 min', steps: 'Complete exercises 1\u20136 (ankle raises, squats, lunges, static holds). Then lie supine in butterfly position (knees out, soles together). Allow tremoring to arise and travel freely. End with 5 min savasana. Neither force nor suppress the tremor.', note: 'Start with 15 min max. Not for acute psychiatric crisis.' },
              { name: 'Constructive Rest', time: '15 min', steps: 'Firm surface, knees bent, small book under head. No effort. Notice where the back contacts the floor and where it doesn\u2019t. Spine decompresses. Roll to one side before standing.', note: 'The Alexander Technique\u2019s daily practice. Deceptively powerful.' },
              { name: 'The Voo Sound', time: '3\u20135 min', steps: 'Inhale through the nose. Exhale sustaining a low-pitched "Voooooo" \u2014 like a foghorn. Feel vibration in chest and belly. 3\u20135 repetitions. Activates the vagus nerve through laryngeal and thoracic vibration.', note: 'Peter Levine\u2019s go-to self-regulation technique.' },
              { name: 'Orienting Exercise', time: '2\u20133 min', steps: 'Slowly turn head and eyes around the room as if arriving somewhere new. Let the eyes be drawn to whatever is interesting. Rest there. The nervous system\u2019s "it\u2019s safe to look" signal.', note: 'Activates the orienting response. Interrupts freeze.' },
              { name: 'Containment', time: '5 min', steps: 'One hand on chest, one on belly. Eyes closed. Three slow breaths. Name what you physically feel \u2014 not what you think: "tightness under my right hand." Affect labeling reduces amygdala reactivity.', note: 'Lieberman et al. 2007: naming sensations downregulates threat.' },
            ].map(p => (
